    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->mainSplitter->restoreGeometry(registrySettings.value("mainsplittergeometry").toByteArray());
    ui->mainSplitter->restoreState(registrySettings.value("mainsplitterstate").toByteArray());
    ...
    connect(ui->clientsTableView->horizontalHeader(),SIGNAL(sectionClicked(int)),this,SLOT(On_clientsHeaderView_sectionClicked()));
//    connect(ui->clientsTableView->selectionModel(),SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),SLOT(On_clientsTableView_selectionChanged(const QItemSelection &, const QItemSelection &)));
// open db and refresh tables.
    ...
    if(!m_adminFlag)
    {
		...
		ui->directionsAction->setVisible(false);
    }
    refreshTypeComboBox();
...
// С Т О Л Б Ц Ы
	...
	ui->columnDirectionAction->setChecked(!ui->requestsTableView->isColumnHidden(3));
	ui->columnAddressAction->setChecked(!ui->requestsTableView->isColumnHidden(4));
	ui->columnSurnameAction->setChecked(!ui->requestsTableView->isColumnHidden(5));
	ui->columnNameAction->setChecked(!ui->requestsTableView->isColumnHidden(6));
	ui->columnPhone1Action->setChecked(!ui->requestsTableView->isColumnHidden(7));
	ui->columnPhone2Action->setChecked(!ui->requestsTableView->isColumnHidden(8));
//    ui->columnDeviceTypeAction->setChecked(!ui->requestsTableView->isColumnHidden(9));
	ui->columnDeviceNameAction->setChecked(!ui->requestsTableView->isColumnHidden(10));
	ui->columnDefectAction->setChecked(!ui->requestsTableView->isColumnHidden(11));
	ui->columnCostAction->setChecked(!ui->requestsTableView->isColumnHidden(12));
	ui->columnPerformerAction->setChecked(!ui->requestsTableView->isColumnHidden(13));
	ui->columnCreatorAction->setChecked(!ui->requestsTableView->isColumnHidden(14));
	ui->columnStatusAction->setChecked(!ui->requestsTableView->isColumnHidden(15));
	ui->columnCollectionAction->setChecked(!ui->requestsTableView->isColumnHidden(16));
...
void MainWindow::closeEvent(QCloseEvent *)
{
    ...
    QSettings registrySettings(ORGNAME,PROGNAME);
    registrySettings.setValue("mainsplittergeometry", ui->mainSplitter->saveGeometry());
    registrySettings.setValue("mainsplitterstate", ui->mainSplitter->saveState());
...
//        qDebug("finded remind");
		...
		RemindDialog *remindDialog = new RemindDialog(0);
		remindDialog->setWindowFlag(Qt::WindowStaysOnTopHint);
...
{
	if(!globaluid) // Не даём запрашивать заявки пока не будем залогинены.
	{
		return;
	}
	debug("refreshRequests()");
    // S O R T
...
//    QMessageBox::information(this,PROGNAME,tr("Фильтр: ")+filter);
	...
	QueryModelExec(this,"SELECT requests.id AS requestsid, requests.ctime AS rctime, performdate, directions.name AS direction_name, clients.address, clients.surname, clients.name AS cname, clients.phone1, clients.phone2, devicename, defect, cost, performer.name AS pname, users.name AS uname, states.name, requests.collection FROM requests LEFT JOIN states ON states.id=requests.status LEFT JOIN clients ON clients.id = requests.clientid LEFT JOIN directions ON directions.id=clients.directionid LEFT JOIN users ON users.id = requests.cuserid LEFT JOIN users AS performer ON performer.id = requests.performerid"+filter+""+sortString+";",requestsQueryModel);
    int column_cnt=0;
	...
	requestsQueryModel->setHeaderData(column_cnt++, Qt::Horizontal, tr("Направление"));
	...
	int masterSumColumn=column_cnt;
...
//    ui->requestsTableView->resizeColumnsToContents();
    ...
    if(m_adminFlag)
    {
        ...
        {
			total+=requestsQueryModel->record(i).value(masterSumColumn).toDouble();
        }
...
{
    if(ui->tabWidget->currentIndex()==TAB_REQUESTS)
    {
        refreshRequests();
    }
    else if(ui->tabWidget->currentIndex()==TAB_AGREEMENTS)
    {
        refreshAgreements();
    }
    else if(ui->tabWidget->currentIndex()==TAB_CLIENTS)
    {
        refreshClients();
    }
}
...
//        filter+=QString((filter.isEmpty())?" WHERE":" AND")+" agreements.type='"+num(ui->agrFilterTypeComboBox->currentData().toInt())+"'";
	...
	if(ui->agrFilterTypeComboBox->currentData().toInt()>0)
	{
		int types=1<<(ui->agrFilterTypeComboBox->currentData().toInt()-1);
		filter+=QString((filter.isEmpty())?" WHERE":" AND")+" agreements.types&"+num(types);
	}
	// Trest
    ...
    if(currentTreeWidgetItem)
    {
        QString dateStr=currentTreeWidgetItem->data(0,Qt::UserRole).toString();
        if(!dateStr.isEmpty())
            filter+=QString((filter.isEmpty())?" WHERE":" AND")+" agreements.alertdate LIKE '"+dateStr+"%'";
    }
	debug("filter: "+filter);
//    QMessageBox::information(this,PROGNAME,tr("Фильтр: ")+filter);
    QModelIndex savedIndex=ui->agreementsTableView->currentIndex();
	QueryModelExec(this,"SELECT agreements.id AS agreementsid, REPLACE(LEFT(startdate, 10),'-','.'), REPLACE(LEFT(alertdate, 10),'-','.'), timelimit, trests.name AS trestname, clients.address, clients.surname, clients.name AS clientname, clients.phone1 AS clientphone1, clients.phone2 AS clientphone2, cost, users.name, CASE WHEN status = 1 THEN 'обслуживается' WHEN status = 2 THEN 'расторгнут' END AS status, collected FROM agreements LEFT JOIN clients ON clients.id = agreements.client LEFT JOIN users ON users.id = agreements.cuser LEFT JOIN trests ON trests.id=agreements.trest"+filter+sortString+";",agreementsQueryModel);
//    qDebug(agreementsQueryModel->query().lastQuery().toUtf8());
	...
	agreementsQueryModel->setHeaderData(column_cnt++, Qt::Horizontal, tr("Телефон 1"));
	agreementsQueryModel->setHeaderData(column_cnt++, Qt::Horizontal, tr("Телефон 2"));
...
void MainWindow::refreshClients()
{
    ...
    if(currentTreeWidgetItem)
    {
        QString dateStr=currentTreeWidgetItem->data(0,Qt::UserRole).toString();
        if(!dateStr.isEmpty())
            filter+=QString((filter.isEmpty())?" WHERE":" AND")+" clients.next_call_date LIKE '"+dateStr+"%'";
    }
    // FilterLineEdit
    QString filterLineEdit=escapeQuotes(ui->filterLineEdit->text());
    if(!filterLineEdit.isEmpty())
    {
        filter+=QString((filter.isEmpty())?" WHERE":" AND")+" (clients.id='"+filterLineEdit+"' OR clients.surname LIKE '%"+filterLineEdit+"%' OR clients.name LIKE '%"+filterLineEdit+"%' OR clients.address LIKE '%"+filterLineEdit+"%' OR clients.phone1 LIKE '%"+filterLineEdit+"%' OR clients.phone2 LIKE '%"+filterLineEdit+"%')";
    }

    QModelIndex savedIndex=ui->clientsTableView->currentIndex();
	QueryModelExec(this,"SELECT clients.id AS clientid, clients.surname AS clientsurname, clients.name AS clientname, clients.phone1, clients.address, CASE WHEN call_result = 0 THEN '-' WHEN call_result = 1 THEN 'Оформили ТО' WHEN call_result = 2 THEN 'Думают' WHEN call_result = 3 THEN 'Отказались' WHEN call_result = 4 THEN 'Не звонить' WHEN call_result = 5 THEN 'Не отвечают' END AS call_result_text, next_call_date, call_result FROM clients"+filter+sortString+";",clientsQueryModel); // ORDER BY call_result, next_call_date, id
...
//-------------------------------------------------------------------------------------
void MainWindow::On_clientsHeaderView_sectionClicked()
{ // Сортировка по выбранной колонке.
    refreshClients();
}
//-------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
void MainWindow::on_columnDirectionAction_triggered()
{
	if(!ui->columnDirectionAction->isChecked())
	{
...
void MainWindow::on_columnCollectionAction_triggered()
{
    if(!ui->columnCollectionAction->isChecked())
		ui->requestsTableView->hideColumn(15);
    else
		ui->requestsTableView->showColumn(15);
}
//-------------------------------------------------------------------------------------
...
/*    int userid=0;
	...
	PrintDialog *printDialog = new PrintDialog(html.toUtf8(),QUrl(),this);
    printDialog->exec();
    delete printDialog;
}
//-------------------------------------------------------------------------------------
...
void MainWindow::on_backupAction_triggered()
{
	...
	QString globalConnectionNameUnderscored=glo.connectionName;
    globalConnectionNameUnderscored.replace(" ","_");
	if(settings.value("connection",glo.connectionName,"dbtype","")=="mysql")
    {
        ...
        if(fileName.isEmpty())
            return;
		QString dbuser=settings.value("connection",glo.connectionName,"dbusername","");
		QString dbpass=settings.value("connection",glo.connectionName,"dbpassword","");
		QString dbhost=settings.value("connection",glo.connectionName,"dbhostname","");
		QString dbname=settings.value("connection",glo.connectionName,"databasename","");
		...
		{
			debug("external program started");
		}
	...
	if(settings.value("connection",glo.connectionName,"dbtype","")=="sqlite")
    {
        ...
        if(fileName.isEmpty())
            return;
		QString dbfilename=settings.value("connection",glo.connectionName,"databasename","");
...
/*    PerformerbillsDialog *performerbillsDialog = new PerformerbillsDialog(this);
	...
	html.replace("[BILLS]",tables.toUtf8());
...
//-------------------------------------------------------------------------------------
void MainWindow::on_directionsAction_triggered()
{
	DirectionsDialog *directionsDialog = new DirectionsDialog(this);
	directionsDialog->exec();
	delete directionsDialog;
}
//-------------------------------------------------------------------------------------
